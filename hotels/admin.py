...
class HotelAdminForm(forms.ModelForm):
    supported_currencies = forms.MultipleChoiceField(
        choices=COMMON_CURRENCIES,
        required=False,
        widget=forms.CheckboxSelectMultiple,
        help_text="Select all currencies this hotel accepts. The default currency is added automatically.",
    )
    ...
    class Meta:
        ...
        fields = "__all__"
        widgets = {
            "brand_color_primary": forms.TextInput(
                attrs={
                    "type": "color",
                    "style": "width: 60px; height: 40px; cursor: pointer;",
                }
            ),
            "brand_color_secondary": forms.TextInput(
                attrs={
                    "type": "color",
                    "style": "width: 60px; height: 40px; cursor: pointer;",
                }
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.pk:
            existing = self.instance.supported_currencies or []
            if isinstance(existing, str):
                existing = [item.strip() for item in existing.replace(";", ",").split(",")]
            existing_codes = [str(code or "").strip().upper()[:3] for code in existing]
            if self.instance.default_currency:
                existing_codes.append(str(self.instance.default_currency).strip().upper()[:3])

        currency_choices = list(COMMON_CURRENCIES)
        known_codes = {code for code, _label in currency_choices}
        for code in existing_codes:
            if len(code) == 3 and code not in known_codes:
                currency_choices.append((code, f"{code} - Custom currency"))
                known_codes.add(code)

        if "default_currency" in self.fields:
            self.fields["default_currency"].widget = forms.Select(choices=currency_choices)

        self.fields["supported_currencies"].choices = currency_choices

        if self.instance and self.instance.pk:
            self.initial["supported_currencies"] = existing_codes or [self.instance.default_currency] or ["UGX"]
        else:
            self.initial["supported_currencies"] = ["UGX"]

    def clean(self):
        cleaned = super().clean()

        default = str(cleaned.get("default_currency") or "UGX").strip().upper()[:3]

        currencies = []
        for code in cleaned.get("supported_currencies") or []:
            code = str(code or "").strip().upper()[:3]
            if len(code) == 3 and code not in currencies:
        ...
        if not cleaned.get("default_currency_symbol"):
            cleaned["default_currency_symbol"] = default
...
class HotelCategoryAdmin(admin.ModelAdmin):
    ...
    def star_rating_range(self, obj):
        ...
        if obj.star_rating_min:
            return f"{obj.star_rating_min}+ ★"
        if obj.star_rating_max:
...
class HotelAdmin(admin.ModelAdmin):
    ...
    def supported_currencies_display(self, obj):
        ...
        if not currencies:
            return obj.default_currency or "-"
...
class HotelExperienceImageInline(admin.TabularInline):
...
class HotelExperienceAdmin(admin.ModelAdmin):
    ...
    list_filter = ("is_approved", "rating", "created_at", "hotel")
    search_fields = (
        "guest_name",
        "guest_email",
        "place_visited",
    ...
    readonly_fields = ("created_at", "updated_at")
    inlines = [HotelExperienceImageInline]
    ordering = ("-created_at",)
    ...
    actions = ["approve_experiences", "unapprove_experiences"]

    def approve_experiences(self, request, queryset):
        updated = queryset.update(is_approved=True)
        self.message_user(request, f"{updated} experiences approved.")

    approve_experiences.short_description = "Approve selected experiences"

    def unapprove_experiences(self, request, queryset):
        updated = queryset.update(is_approved=False)
        self.message_user(request, f"{updated} experiences unapproved.")
